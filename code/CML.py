...
def func_renew_node_state(matrix_flow: np.ndarray, node_state_dict: dict, node_list: list, miu: tuple):
    ...
    for i in node_list:
        ...
        re_1, re_2, i_in_sum, i_out_sum = 0, 0, np.sum(matrix_flow[:, node_list.index(i)]), np.sum(matrix_flow[node_list.index(i), :])
        for j in node_list:
            if i != j:
                re_1 += (matrix_flow[node_list.index(i), node_list.index(j)] * 4 * node_state_dict[j] * (1 - node_state_dict[j])) / (i_out_sum+0.0000001)
                re_2 += (matrix_flow[node_list.index(j), node_list.index(i)] * 4 * node_state_dict[j] * (1 - node_state_dict[j])) / (i_in_sum+0.0000001)
